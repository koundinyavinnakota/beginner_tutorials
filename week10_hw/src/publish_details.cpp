...
public:
  DetailsPublisher(): Node("details_publisher")
  {
    ...
    service_ = this->create_service<week10_hw::srv::Details>("change_details",std::bind(&DetailsPublisher))
...
private:
  ...
  void timer_callback() {    
    ...
    details_publisher_->publish(message)
  }
  ...
  void change_details(const std::shared_ptr<week10_hw::srv::Details::Request> request,
   std::shared_ptr<week10_hw::srv::Details::Response> response){
    ...
    RCLCPP_INFO(this->get_logger(),
    " Changed Details Request \n First Name: %s " "\nLast Name: %s " "\nAge: %d ",response.first_name.c_str(), response.last_name.c_str(), response.age);
  }

  rclcpp::Publisher<week10_hw::msg::Details>::SharedPtr details_publisher;
  rclcpp::TimerBase::SharedPtr timer_;
  size_t count_;
  auto message_ = week10_hw::msg::Details();// Custom message to publish
